...
class Minimercado
{
    ...
    static void ImportarInventarioExcel()
    {
        ...
        Console.WriteLine("El archivo debe tener columnas: ID | Nombre | Categoria | Precio | Stock");
        Console.WriteLine();

        Console.Write("Nombre del archivo (ej: inventario.xlsx): ");
        string nombreArchivo = Console.ReadLine();

        if (File.Exists(nombreArchivo) == false)
        {
            Console.WriteLine($"El archivo '{nombreArchivo}' no se encontro.");
            Console.WriteLine($"Asegurate de que este en: {Directory.GetCurrentDirectory()}");
            return;
        }
        ...
        foreach (IXLRow fila in hoja.RowsUsed().Skip(1))
        {
            ...
            { Console.WriteLine($"Fila {fila.RowNumber()}: ID invalido, se omite."); errores = errores + 1; continue; }
            ...
            { Console.WriteLine($"Fila {fila.RowNumber()}: Precio invalido, se omite."); errores = errores + 1; continue; }

            if (int.TryParse(celdaStock, out int stock) == false || stock < 0)
            { Console.WriteLine($"Fila {fila.RowNumber()}: Stock invalido, se omite."); errores = errores + 1; continue; }
        ...
        {
            Console.WriteLine("No se encontraron productos validos. El catalogo no fue modificado.");
            return;
        }
        ...
        Console.WriteLine("Catalogo actualizado.");
    }
